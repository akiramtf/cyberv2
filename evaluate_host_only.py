"""Evaluate model using ONLY host-based features (18 features)"""
...
# Define host-based features to keep
HOST_FEATURES = {
    # DNS features (8)
...
def evaluate_host_only(file_path: str, test_size: float = 0.2):
    """Evaluate model using only host-based features"""
    
    print("=" * 70)
    print("HOST-BASED-ONLY FEATURE EVALUATION (18 features)")
    print("=" * 70)
    print("⚠️  WARNING: This will be SLOW due to DNS/SSL lookups!")
    print("=" * 70)
    ...
    # 2. Extract Features (host-based - DNS/SSL ENABLED)
    logger.info("\n🔍 Extracting HOST-BASED features only (DNS/SSL enabled)...")
    logger.info("   ⏳ This may take several minutes due to network lookups...")
    ...
    for i, (url, label) in enumerate(zip(urls, labels)):
        try:
            if i % 10 == 0:
                print(f"   Processing {i}/{len(urls)}... (this is slow)", end="\r")
    ...
    # 3. Filter to host-based features only
    df_features = pd.DataFrame(features_list)
    
    # Keep only host-based features
    ...
    logger.info(f"✓ Extracted {len(available_host_features)} host-based features")
    logger.info(f"   Expected: {len(HOST_FEATURES)} features")
    ...
    # 5. Train model on host-based features only
    logger.info("\n🎯 Training XGBoost on HOST-BASED features only...")
    model = XGBClassifier(
        n_estimators=100,
        max_depth=6,
        learning_rate=0.1,
    ...
    # Print Results
    print("\n" + "=" * 70)
    print("HOST-BASED-ONLY EVALUATION RESULTS")
    print("=" * 70)
    print(f"Features Used:       {len(available_host_features)} HOST-BASED features only")
    ...
    # Feature importance
    print("\n" + "=" * 70)
    print("TOP 10 MOST IMPORTANT HOST-BASED FEATURES")
    print("=" * 70)
    feature_importance = sorted(
        zip(available_host_features, model.feature_importances_),
        key=lambda x: x[1],
        reverse=True
    )
    ...
    if misclassified:
        ...
        error_file = "misclassified_host_only.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate model using only host-based features")
    parser.add_argument("data", type=str, help="Path to test data CSV file")
    parser.add_argument("--test-size", type=float, default=0.2, help="Test set size (default: 0.2)")
    
    args = parser.parse_args()
    evaluate_host_only(args.data, args.test_size)
